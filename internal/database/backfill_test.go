...
func TestRunBackfillIsResumable(t *testing.T) {
	...
	// UpsertItem derives item_text on write, so the seeded items are already
	// indexed. Discard that to reproduce the case this test is about: existing
	// items whose derived text is missing or stale, which is what gives the
	// interrupted backfill real work to interrupt.
